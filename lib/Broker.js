...
var BrokerRequest = require('./BrokerRequest');
...
Broker.prototype.onClient = function(msg) {
  ...
  } else if (type == MDP.W_META) {
    var srvName = msg[3];
    var rid = msg[4];
    var query = msg[5];
    if (query === 'service:workers') {
      var service = this.services[srvName]; 
      var workers = service ? service.workers : [];
      this.send([clientId, MDP.CLIENT, MDP.W_REPLY, '', rid, 0, JSON.stringify(workers)]);
      return;
    }
    this.send([clientId, MDP.CLIENT, MDP.W_REPLY, rid, -1, '"C_NOQUERY"', '{}']);
  }
};
...
Broker.prototype.requestProcess = function(req) {
  ...
  var workerId = req.opts && req.opts.workerId;
  var service = this.serviceRequire(req.service);
  if (!workerId) { 
    service.q.push(req);
    ...
    this.workerDispatch(req.service, workerId);
  }
};
...
function _rhandle(srvName, workerIds) {
  ...
  var q = this.serviceRequire(srvName).q;
...
Broker.prototype.workerDispatch = function (srvName, workerId) {
  _rhandle.call(this, srvName, [workerId]);
}
